...
jq --argjson groupInfo "$(echo $verkey)" '. + $groupInfo' $(ls -t runtime/output/*.batch_proof.public.json | head -n1) > $(ls -t runtime/output/*.batch_proof.public.json | head -n1).tmp && mv $(ls -t runtime/output/*.batch_proof.public.json | head -n1).tmp $(ls -t runtime/output/*.batch_proof.public.json | head -n1)
