...
#include "circom.multichainAgg.hpp"
...
#define handle_error(msg) \
  ...
  void getCommitedPols(CommitPolsStarks *commitPols, const std::string multichainAggVerifier, const std::string execFile, nlohmann::json &zkin, uint64_t N, uint64_t nCols)
  {
    //-------------------------------------------
    // Verifier stark proof
    //-------------------------------------------
    TimerStart(CIRCOM_LOAD_CIRCUIT_PROOF_MULTICHAIN_AGG);
    Circom_Circuit *circuit = loadCircuit(multichainAggVerifier);
    TimerStopAndLog(CIRCOM_LOAD_CIRCUIT_PROOF_MULTICHAIN_AGG);
    TimerStart(CIRCOM_LOAD_JSON_MULTICHAIN_AGG_PROOF);
    Circom_CalcWit *ctx = new Circom_CalcWit(circuit);

    loadJsonImpl(ctx, zkin);
    if (ctx->getRemaingInputsToBeSet() != 0)
    {
      zklog.error("Prover::genMultichainAggProof() Not all inputs have been set. Only " + to_string(get_main_input_signal_no() - ctx->getRemaingInputsToBeSet()) + " out of " + to_string(get_main_input_signal_no()));
      exitProcess();
    }
    TimerStopAndLog(CIRCOM_LOAD_JSON_MULTICHAIN_AGG_PROOF);
    //-------------------------------------------
    // Compute witness and commited pols
    //-------------------------------------------
    TimerStart(STARK_WITNESS_AND_COMMITED_POLS_MULTICHAIN_AGG_PROOF);
    ...
    delete[] tmp;
    freeCircuit(circuit);
    TimerStopAndLog(STARK_WITNESS_AND_COMMITED_POLS_MULTICHAIN_AGG_PROOF);
  }
}
