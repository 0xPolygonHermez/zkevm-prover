...
#define handle_error(msg) \
  ...
  void getCommitedPols(CommitPolsStarks *commitPols, const std::string multichainAggFVerifier, const std::string execFile, nlohmann::json &zkin, uint64_t N, uint64_t nCols)
  {
    ...
    //-------------------------------------------
    TimerStart(CIRCOM_LOAD_CIRCUIT_PROOF_MULTICHAIN_AGG_F);
    Circom_Circuit *circuit = loadCircuit(multichainAggFVerifier);
    TimerStopAndLog(CIRCOM_LOAD_CIRCUIT_PROOF_MULTICHAIN_AGG_F);
    TimerStart(CIRCOM_LOAD_JSON_MULTICHAIN_AGG_F_PROOF);
    Circom_CalcWit *ctx = new Circom_CalcWit(circuit);

    loadJsonImpl(ctx, zkin);
    if (ctx->getRemaingInputsToBeSet() != 0)
    {
      zklog.error("Prover::genMultichainAggFProof() Not all inputs have been set. Only " + to_string(get_main_input_signal_no() - ctx->getRemaingInputsToBeSet()) + " out of " + to_string(get_main_input_signal_no()));
      exitProcess();
    }
    TimerStopAndLog(CIRCOM_LOAD_JSON_MULTICHAIN_AGG_F_PROOF);
    //-------------------------------------------
    // Compute witness and commited pols
    //-------------------------------------------
    TimerStart(STARK_WITNESS_AND_COMMITED_POLS_MULTICHAIN_AGG_F_PROOF);
    ...
    delete[] tmp;
    freeCircuit(circuit);
    TimerStopAndLog(STARK_WITNESS_AND_COMMITED_POLS_MULTICHAIN_AGG_F_PROOF);
  }
