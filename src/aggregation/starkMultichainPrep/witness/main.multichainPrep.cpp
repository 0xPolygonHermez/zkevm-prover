...
#define handle_error(msg) \
  ...
  void getCommitedPols(CommitPolsStarks *commitPols, const std::string multichainPrepVerifier, const std::string execFile, nlohmann::json &zkin, uint64_t N, uint64_t nCols, nlohmann::json &outHashInfo)
  {
    ...
    //-------------------------------------------
    TimerStart(CIRCOM_LOAD_CIRCUIT_PROOF_MULTICHAIN_PREP);
    Circom_Circuit *circuit = loadCircuit(multichainPrepVerifier);
    TimerStopAndLog(CIRCOM_LOAD_CIRCUIT_PROOF_MULTICHAIN_PREP);
    TimerStart(CIRCOM_LOAD_JSON_MULTICHAIN_PREP_PROOF);
    Circom_CalcWit *ctx = new Circom_CalcWit(circuit);

    loadJsonImpl(ctx, zkin);
    if (ctx->getRemaingInputsToBeSet() != 0)
    {
      zklog.error("Prover::genMultichainPrepProof() Not all inputs have been set. Only " + to_string(get_main_input_signal_no() - ctx->getRemaingInputsToBeSet()) + " out of " + to_string(get_main_input_signal_no()));
      exitProcess();
    }
    TimerStopAndLog(CIRCOM_LOAD_JSON_MULTICHAIN_PREP_PROOF);

    //-------------------------------------------
    // Compute witness and commited pols
    //-------------------------------------------
    TimerStart(STARK_WITNESS_AND_COMMITED_POLS_MULTICHAIN_PREP_PROOF);
    ...
    outHashInfo["nBlocks"] = Goldilocks::toU64(tmp[1]);
    outHashInfo["outHash"] = json::array();
    for(uint64_t i = 0; i < 8; i++) {
      outHashInfo["outHash"][i] = Goldilocks::toU64(tmp[2 + i]);
    }
    ...
    delete[] tmp;
    freeCircuit(circuit);
    TimerStopAndLog(STARK_WITNESS_AND_COMMITED_POLS_MULTICHAIN_PREP_PROOF);
  }
