...
public:
    ...
    Executor(Goldilocks &fr, const Config &config, PoseidonGoldilocks &poseidon, int mpiRank_ = 0) :
        fr(fr),
        config(config),
        mainExecutor_fork_1(fr, poseidon, config, mpiRank_),
        mainExecutor_fork_2(fr, poseidon, config, mpiRank_),
        mainExecutor_fork_3(fr, poseidon, config, mpiRank_),
        mainExecutor_fork_4(fr, poseidon, config, mpiRank_),
        mainExecutor_fork_5(fr, poseidon, config, mpiRank_),
        mainExecutor_fork_6(fr, poseidon, config, mpiRank_),
        mainExecutor_fork_7(fr, poseidon, config, mpiRank_),
        mainExecutorC_fork_7(mainExecutor_fork_7),
        storageExecutor(fr, poseidon, config, mpiRank_),
        memoryExecutor(fr, config),
        binaryExecutor(fr, config, mpiRank_),
        arithExecutor(fr, config),
        paddingKKExecutor(fr),
        paddingKKBitExecutor(fr),
        bits2FieldExecutor(fr),
        keccakFExecutor(fr, config, mpiRank_),
        paddingSha256Executor(fr),
        paddingSha256BitExecutor(fr),
        bits2FieldSha256Executor(fr),
        sha256FExecutor(fr, config, mpiRank_),
        paddingPGExecutor(fr, poseidon),
        poseidonGExecutor(fr, poseidon, mpiRank_),
