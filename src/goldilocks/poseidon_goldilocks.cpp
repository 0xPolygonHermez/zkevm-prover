...
{
    tree[0] = Goldilocks::fromU64(1);
    tree[1] = Goldilocks::fromU64(num_cols);
    tree[2] = Goldilocks::fromU64(num_rows);

    std::memcpy(&tree[MERKLEHASHGOLDILOCKS_HEADER_SIZE], input, num_cols * num_rows * sizeof(Goldilocks::Element));

    Goldilocks::Element *cursor = &tree[MERKLEHASHGOLDILOCKS_HEADER_SIZE + num_cols * num_rows];
    ...
    for (uint64_t i = 0; i < num_rows; i++)
    {
        ...
        linear_hash(&cursor[i * CAPACITY], intermediate, num_cols);
    }
