...
string generate(const json &rom, uint64_t forkID, string forkNamespace, const string &functionName, const string &fileName, bool bFastMode, bool bHeader)
{
    ...
    for (uint64_t zkPC=0; zkPC < rom["program"].size(); zkPC++)
    {
        ...
        // If bJumpToZkPC=true, the code will jump to pols.zkPC 
        bool bJumpToZkPC = false;
        ...
        if (rom["program"][zkPC].contains("inRID") && (rom["program"][zkPC]["inRID"]!=0))
        {
        ...
        if (rom["program"][zkPC].contains("inFREE"))
        {
                 ...
                 (rom["program"][zkPC]["freeInTag"]["op"] == "") )
            {
                ...
                if (rom["program"][zkPC].contains("hashK") && (rom["program"][zkPC]["hashK"] == 1))
                {
                    ...
                    code += "    {\n";
                    code += "        ctx.hashK[hashAddr] = emptyHashValue;\n";
                    code += "        hashIterator = ctx.hashK.find(hashAddr);\n";
                ...
                if (rom["program"][zkPC].contains("hashKDigest") && (rom["program"][zkPC]["hashKDigest"] == 1))
                {
                    ...
                    code += "        zkPC=" + to_string(zkPC) +";\n";
                    code += "        mainExecutor.logError(ctx, \"HashKDigest 1: digest not defined for addr=\" + to_string(hashAddr));\n";
                    code += "        pHashDB->cancelBatch(proverRequest.uuid);\n";
                    code += "        return;\n";
                    code += "    }\n";
                    ...
                    code += "        zkPC=" + to_string(zkPC) +";\n";
                    code += "        mainExecutor.logError(ctx, \"HashKDigest 1: digest not calculated for addr=\" + to_string(hashAddr) + \".  Call hashKLen to finish digest.\");\n";
                    code += "        pHashDB->cancelBatch(proverRequest.uuid);\n";
                    code += "        return;\n";
                    code += "    }\n";
                ...
                if ( rom["program"][zkPC].contains("hashP") && (rom["program"][zkPC]["hashP"] == 1))
                {
                    ...
                    code += "    {\n";
                    code += "        ctx.hashP[hashAddr] = emptyHashValue;\n";
                    code += "        hashIterator = ctx.hashP.find(hashAddr);\n";
                ...
                if (rom["program"][zkPC].contains("hashPDigest") && (rom["program"][zkPC]["hashPDigest"] == 1))
                {
                    ...
                    code += "        zkPC=" + to_string(zkPC) +";\n";
                    code += "        mainExecutor.logError(ctx, \"HashPDigest 1: digest not defined addr=\" + to_string(hashAddr));\n";
                    code += "        pHashDB->cancelBatch(proverRequest.uuid);\n";
                    code += "        return;\n";
                    code += "    }\n";
                    //code += "    // If digest was not calculated, this is an error\n";
                ...
                if (rom["program"][zkPC].contains("hashS") && (rom["program"][zkPC]["hashS"] == 1))
                {
                    ...
                    code += "    {\n";
                    code += "        ctx.hashS[hashAddr] = emptyHashValue;\n";
                    code += "        hashIterator = ctx.hashS.find(hashAddr);\n";
                ...
                if (rom["program"][zkPC].contains("hashSDigest") && (rom["program"][zkPC]["hashSDigest"] == 1))
                {
                    ...
                    code += "        zkPC=" + to_string(zkPC) +";\n";
                    code += "        mainExecutor.logError(ctx, \"HashSDigest 1: digest not defined for addr=\" + to_string(hashAddr));\n";
                    code += "        pHashDB->cancelBatch(proverRequest.uuid);\n";
                    code += "        return;\n";
                    code += "    }\n";
                    ...
                    code += "        zkPC=" + to_string(zkPC) +";\n";
                    code += "        mainExecutor.logError(ctx, \"HashSDigest 1: digest not calculated for addr=\" + to_string(hashAddr) + \".  Call hashSLen to finish digest.\");\n";
                    code += "        pHashDB->cancelBatch(proverRequest.uuid);\n";
                    code += "        return;\n";
                    code += "    }\n";
                ...
                if (rom["program"][zkPC].contains("arith") &&
        ...
        if (rom["program"][zkPC].contains("hashK") && (rom["program"][zkPC]["hashK"] == 1))
        {
            ...
            code += "        proverRequest.result = ZKR_SM_MAIN_FEA2SCALAR;\n";
            code += "        zkPC=" + to_string(zkPC) +";\n";
            code += "        mainExecutor.logError(ctx, \"Failed calling fea2scalar(" + string((rom["program"][zkPC].contains("assumeFree") && (rom["program"][zkPC]["assumeFree"] == 1)) ? "pols.FREE" : "op") + ")\");\n";
            code += "        pHashDB->cancelBatch(proverRequest.uuid);\n";
            code += "        return;\n";
            code += "    }\n\n";
            ...
            code += "            proverRequest.result = ZKR_SM_MAIN_HASHK_POSITION_PLUS_SIZE_OUT_OF_RANGE;\n";
            code += "            zkPC=" + to_string(zkPC) +";\n";
            code += "            mainExecutor.logError(ctx, \"HashK 2: trying to insert data in a position:\" + to_string(pos+j) + \" higher than current data size:\" + to_string(ctx.hashK[hashAddr].data.size()));\n";
            code += "            pHashDB->cancelBatch(proverRequest.uuid);\n";
            code += "            return;\n";
            code += "        }\n";
            code += "        else\n";
            ...
            code += "                zkPC=" + to_string(zkPC) +";\n";
            code += "                mainExecutor.logError(ctx, \"HashK 2 bytes do not match: addr=\" + to_string(hashAddr) + \" pos+j=\" + to_string(pos+j) + \" is bm=\" + to_string(bm) + \" and it should be bh=\" + to_string(bh));\n";
            code += "                pHashDB->cancelBatch(proverRequest.uuid);\n";
            code += "                return;\n";
            code += "            }\n";
            code += "        }\n";
            ...
            code += "            zkPC=" + to_string(zkPC) +";\n";
            code += "            mainExecutor.logError(ctx, \"HashK 2 different read sizes in the same position addr=\" + to_string(hashAddr) + \" pos=\" + to_string(pos) + \" ctx.hashK[addr].reads[pos]=\" + to_string(ctx.hashK[hashAddr].reads[pos]) + \" size=\" + to_string(size));\n";
            code += "            pHashDB->cancelBatch(proverRequest.uuid);\n";
            code += "            return;\n";
            code += "        }\n";
            code += "    }\n";
...
#ifdef LOG_HASHK
            code += "    zklog.info(\"hashK 2 i=\" + to_string(i) + \" zkPC=" + to_string(zkPC) + " addr=\" + to_string(hashAddr) + \" pos=\" + to_string(pos) + \" size=\" + to_string(size) + \" data=\" + a.get_str(16));\n";
#endif
        }
        ...
        if (rom["program"][zkPC].contains("hashKLen") && (rom["program"][zkPC]["hashKLen"] == 1))
        {
            ...
            code += "            zkPC=" + to_string(zkPC) +";\n";
            code += "            mainExecutor.logError(ctx, \"HashKLen 2 hashK[addr] is empty but lm is not 0 addr=\" + to_string(hashAddr) + \" lm=\" + to_string(lm));\n";
            code += "            pHashDB->cancelBatch(proverRequest.uuid);\n";
            code += "            return;\n";
            code += "        }\n\n";
            ...
            code += "    if (ctx.hashK[hashAddr].lenCalled)\n";
            code += "    {\n";
            code += "        proverRequest.result = ZKR_SM_MAIN_HASHKLEN_CALLED_TWICE;\n";
            code += "        zkPC=" + to_string(zkPC) +";\n";
            code += "        mainExecutor.logError(ctx, \"HashKLen 2 called more than once addr=\" + to_string(hashAddr));\n";
            code += "        pHashDB->cancelBatch(proverRequest.uuid);\n";
            code += "        return;\n";
            code += "    }\n";
            code += "    ctx.hashK[hashAddr].lenCalled = true;\n";
            ...
            code += "        proverRequest.result = ZKR_SM_MAIN_HASHKLEN_LENGTH_MISMATCH;\n";
            code += "        zkPC=" + to_string(zkPC) +";\n";
            code += "        mainExecutor.logError(ctx, \"HashKLen 2 length does not match addr=\" + to_string(hashAddr) + \" is lm=\" + to_string(lm) + \" and it should be lh=\" + to_string(lh));\n";
            code += "        pHashDB->cancelBatch(proverRequest.uuid);\n";
            code += "        return;\n";
            code += "    }\n";
...
#ifdef LOG_HASHK
            code += "        {\n";
            code += "           string s = \"hashKLen 2 calculate hashKLen: addr:\" + to_string(hashAddr) + \" hash:\" + ctx.hashK[hashAddr].digest.get_str(16) + \" size:\" + to_string(ctx.hashK[hashAddr].data.size()) + \" data:\";\n";
            code += "           for (uint64_t k=0; k<ctx.hashK[hashAddr].data.size(); k++) s += byte2string(ctx.hashK[hashAddr].data[k]) + \":\";\n";
            code += "           zklog.info(s);\n";
            code += "        }\n";
#endif
            code += "    }\n";
...
#ifdef LOG_HASHK
            code += "    zklog.info(\"hashKLen 2 i=\" + to_string(i) + \" zkPC=" + to_string(zkPC) + " addr=\" + to_string(hashAddr));\n";
#endif
        }
        ...
        if (rom["program"][zkPC].contains("hashKDigest") && (rom["program"][zkPC]["hashKDigest"] == 1))
        {
            ...
            code += "        zkPC=" + to_string(zkPC) +";\n";
            code += "        mainExecutor.logError(ctx, \"HashKDigest 2 could not find entry for addr=\" + to_string(hashAddr));\n";
            code += "        pHashDB->cancelBatch(proverRequest.uuid);\n";
            code += "        return;\n";
            code += "    }\n";
            ...
            code += "        zkPC=" + to_string(zkPC) +";\n";
            code += "        mainExecutor.logError(ctx, \"HashKDigest 2 called more than once addr=\" + to_string(hashAddr));\n";
            code += "        pHashDB->cancelBatch(proverRequest.uuid);\n";
            code += "        return;\n";
            code += "    }\n";
        ...
        if (rom["program"][zkPC].contains("hashP") && (rom["program"][zkPC]["hashP"] == 1))
        {
            ...
            code += "                mainExecutor.logError(ctx, \"HashP 2 zero position already existed addr=\" + to_string(hashAddr) + \" pos=\" + to_string(pos));\n";
            code += "                pHashDB->cancelBatch(proverRequest.uuid);\n";
            code += "                return;\n";
            code += "            }\n";
            code += "            else\n";
            ...
            code += "            proverRequest.result = ZKR_SM_MAIN_HASHP_POSITION_PLUS_SIZE_OUT_OF_RANGE;\n";
            code += "            zkPC=" + to_string(zkPC) +";\n";
            code += "            mainExecutor.logError(ctx, \"HashP 2: trying to insert data in a position:\" + to_string(pos+j) + \" higher than current data size:\" + to_string(ctx.hashP[hashAddr].data.size()));\n";
            code += "            pHashDB->cancelBatch(proverRequest.uuid);\n";
            code += "            return;\n";
            code += "        }\n";
            code += "        else\n";
            ...
            code += "                zkPC=" + to_string(zkPC) +";\n";
            code += "                mainExecutor.logError(ctx, \"HashP 2 bytes do not match: addr=\" + to_string(hashAddr) + \" pos+j=\" + to_string(pos+j) + \" is bm=\" + to_string(bm) + \" and it should be bh=\" + to_string(bh));\n";
            code += "                pHashDB->cancelBatch(proverRequest.uuid);\n";
            code += "                return;\n";
            code += "            }\n";
            code += "        }\n";
            ...
            code += "            proverRequest.result = ZKR_SM_MAIN_HASHP_SIZE_MISMATCH;\n";
            code += "            zkPC=" + to_string(zkPC) +";\n";
            code += "            mainExecutor.logError(ctx, \"HashP 2 diferent read sizes in the same position addr=\" + to_string(hashAddr) + \" pos=\" + to_string(pos));\n";
            code += "            pHashDB->cancelBatch(proverRequest.uuid);\n";
            code += "            return;\n";
            code += "        }\n";
            code += "    }\n";
        ...
        if (rom["program"][zkPC].contains("hashPLen") && (rom["program"][zkPC]["hashPLen"] == 1))
        {
            ...
            code += "            zkPC=" + to_string(zkPC) +";\n";
            code += "            mainExecutor.logError(ctx, \"HashPLen 2 hashP[addr] is empty but lm is not 0 addr=\" + to_string(hashAddr) + \" lm=\" + to_string(lm));\n";
            code += "            pHashDB->cancelBatch(proverRequest.uuid);\n";
            code += "            return;\n";
            code += "        }\n\n";
            ...
            code += "        proverRequest.result = ZKR_SM_MAIN_HASHPLEN_LENGTH_MISMATCH;\n";
            code += "        zkPC=" + to_string(zkPC) +";\n";
            code += "        mainExecutor.logError(ctx, \"HashPLen 2 does not match match addr=\" + to_string(hashAddr) + \" is lm=\" + to_string(lm) + \" and it should be lh=\" + to_string(lh));\n";
            code += "        pHashDB->cancelBatch(proverRequest.uuid);\n";
            code += "        return;\n";
            code += "    }\n";
            code += "    if (!hashIterator->second.digestCalled)\n";
...
#ifdef LOG_HASHP
            code += "        {\n";
            code += "           string s = \"Hash calculate hashPLen 2: addr:\" + to_string(hashAddr) + \" hash:\" + ctx.hashP[hashAddr].digest.get_str(16) + \" size:\" + to_string(ctx.hashP[hashAddr].data.size()) + \" data:\";\n";
            code += "           for (uint64_t k=0; k<ctx.hashP[hashAddr].data.size(); k++) s += byte2string(ctx.hashP[hashAddr].data[k]) + \":\";\n";
            code += "           zklog.info(s);\n";
            code += "        }\n";
#endif
            code += "    }\n";
        ...
        if (rom["program"][zkPC].contains("hashPDigest") && (rom["program"][zkPC]["hashPDigest"] == 1))
        {
...
#endif
            code += "        ctx.hashP[hashAddr] = hashValue;\n";
            code += "        hashIterator = ctx.hashP.find(hashAddr);\n";
            code += "        zkassert(hashIterator != ctx.hashP.end());\n";
            code += "    }\n";
            ...
            code += "        zkPC=" + to_string(zkPC) +";\n";
            code += "        mainExecutor.logError(ctx, \"HashPDigest 2 called more than once addr=\" + to_string(hashAddr));\n";
            code += "        pHashDB->cancelBatch(proverRequest.uuid);\n";
            code += "        return;\n";
            code += "    }\n";
            code += "    hashIterator->second.digestCalled = true;\n";
        ...
        if (rom["program"][zkPC].contains("hashS") && (rom["program"][zkPC]["hashS"] == 1))
        {
            ...
            code += "            proverRequest.result = ZKR_SM_MAIN_HASHS_POSITION_PLUS_SIZE_OUT_OF_RANGE;\n";
            code += "            zkPC=" + to_string(zkPC) +";\n";
            code += "            mainExecutor.logError(ctx, \"HashS 2: trying to insert data in a position:\" + to_string(pos+j) + \" higher than current data size:\" + to_string(ctx.hashK[hashAddr].data.size()));\n";
            code += "            pHashDB->cancelBatch(proverRequest.uuid);\n";
            code += "            return;\n";
            code += "        }\n";
            code += "        else\n";
            ...
            code += "                zkPC=" + to_string(zkPC) +";\n";
            code += "                mainExecutor.logError(ctx, \"HashS 2 bytes do not match: addr=\" + to_string(hashAddr) + \" pos+j=\" + to_string(pos+j) + \" is bm=\" + to_string(bm) + \" and it should be bh=\" + to_string(bh));\n";
            code += "                pHashDB->cancelBatch(proverRequest.uuid);\n";
            code += "                return;\n";
            code += "            }\n";
            code += "        }\n";
            ...
            code += "            zkPC=" + to_string(zkPC) +";\n";
            code += "            mainExecutor.logError(ctx, \"HashS 2 different read sizes in the same position addr=\" + to_string(hashAddr) + \" pos=\" + to_string(pos) + \" ctx.hashK[addr].reads[pos]=\" + to_string(ctx.hashK[addr].reads[pos]) + \" size=\" + to_string(size));\n";
            code += "            pHashDB->cancelBatch(proverRequest.uuid);\n";
            code += "            return;\n";
            code += "        }\n";
            code += "    }\n";
...
#ifdef LOG_HASHS
            code += "    zklog.info(\"hashS 2 i=\" + to_string(i) + \" zkPC=" + to_string(zkPC) + " addr=\" + to_string(hashAddr) + \" pos=\" + to_string(pos) + \" size=\" + to_string(size) + \" data=\" + a.get_str(16));\n";
#endif
        }
        ...
        if (rom["program"][zkPC].contains("hashSLen") && (rom["program"][zkPC]["hashSLen"] == 1))
        {
            ...
            code += "            zkPC=" + to_string(zkPC) +";\n";
            code += "            mainExecutor.logError(ctx, \"HashSLen 2 hashK[addr] is empty but lm is not 0 addr=\" + to_string(hashAddr) + \" lm=\" + to_string(lm));\n";
            code += "            pHashDB->cancelBatch(proverRequest.uuid);\n";
            code += "            return;\n";
            code += "        }\n\n";
            ...
            code += "        proverRequest.result = ZKR_SM_MAIN_HASHSLEN_LENGTH_MISMATCH;\n";
            code += "        zkPC=" + to_string(zkPC) +";\n";
            code += "        mainExecutor.logError(ctx, \"HashSLen 2 length does not match addr=\" + to_string(hashAddr) + \" is lm=\" + to_string(lm) + \" and it should be lh=\" + to_string(lh));\n";
            code += "        pHashDB->cancelBatch(proverRequest.uuid);\n";
            code += "        return;\n";
            code += "    }\n";
            code += "    if (!hashIterator->second.digestCalled)\n";
...
#ifdef LOG_HASHS
            code += "        {\n";
            code += "           string s = \"hashSLen 2 calculate hashSLen: addr:\" + to_string(hashAddr) + \" hash:\" + ctx.hashS[hashAddr].digest.get_str(16) + \" size:\" + to_string(ctx.hashS[hashAddr].data.size()) + \" data:\";\n";
            code += "           for (uint64_t k=0; k<ctx.hashS[hashAddr].data.size(); k++) s += byte2string(ctx.hashS[hashAddr].data[k]) + \":\";\n";
            code += "           zklog.info(s);\n";
            code += "        }\n";
#endif
            code += "    }\n";
...
#ifdef LOG_HASHS
            code += "    zklog.info(\"hashSLen 2 i=\" + to_string(i) + \" zkPC=" + to_string(zkPC) + " addr=\" + to_string(hashAddr));\n";
#endif
        }
        ...
        if (rom["program"][zkPC].contains("hashSDigest") && (rom["program"][zkPC]["hashSDigest"] == 1))
        {
            ...
            code += "            return;\n";
            code += "        }\n";
            code += "        ctx.hashS[hashAddr] = hashValue;\n";
            code += "        hashIterator = ctx.hashS.find(hashAddr);\n";
            code += "        zkassert(hashIterator != ctx.hashS.end());\n";
            code += "    }\n";
            ...
            code += "        zkPC=" + to_string(zkPC) +";\n";
            code += "        mainExecutor.logError(ctx, \"HashSDigest 2 called more than once addr=\" + to_string(hashAddr));\n";
            code += "        pHashDB->cancelBatch(proverRequest.uuid);\n";
            code += "        return;\n";
            code += "    }\n";
            code += "    hashIterator->second.digestCalled = true;\n";
        ...
        // Save instruction
        if (rom["program"][zkPC].contains("save") && (rom["program"][zkPC]["save"] == 1))
        {
            code += "    rid = i;\n";
#ifdef LOG_SAVE_RESTORE
        ...
        bool JMP = (rom["program"][zkPC].contains("JMP") && (rom["program"][zkPC]["JMP"]==1));
        ...
        if (rom["program"][zkPC].contains("jmpUseAddrRel") && (rom["program"][zkPC]["jmpUseAddrRel"] == 1))
        {
            jmpUseAddrRel = true;
            if (!bFastMode)
            {
        ...
        if (rom["program"][zkPC].contains("JMPN") && (rom["program"][zkPC]["JMPN"]==1))
        {
            ...
            if (!bFastMode)
            {
                ...
                code += "        jmpnCondValue = fr.toU64(fr.add(o, fr.fromU64(0x100000000)));\n";
            }
        ...
        else if (rom["program"][zkPC].contains("return") && (rom["program"][zkPC]["return"] == 1))
        {
            if (!bFastMode)
            {
                code += "    pols.zkPC[nexti] = pols.RR[nexti];\n"; // This i became nexti in fork 10
