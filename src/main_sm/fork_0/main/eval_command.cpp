...
void eval_eventLog(Context &ctx, const RomCommand &cmd, CommandResult &cr)
{
    ...
    if (ctx.proverRequest.input.publicInputsExtended.publicInputs.forkID == 0)
    {
        ((fork_0::FullTracer *)ctx.proverRequest.pFullTracer)->handleEvent(ctx, cmd);
    }
    else
    {
        cerr << "Error: eval_eventLog() called with fork ID=" << ctx.proverRequest.input.publicInputsExtended.publicInputs.forkID << " different from 0" << endl;
        exitProcess();
    }
...
void eval_storeLog (Context &ctx, const RomCommand &cmd, CommandResult &cr)
{
    ...
    {
        cerr << "Error: eval_storeLog() called with fork ID=" << ctx.proverRequest.input.publicInputsExtended.publicInputs.forkID << " different from 0" << endl;
        exitProcess();
    }
