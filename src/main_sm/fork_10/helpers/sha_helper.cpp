...
zkresult HashS_calculate (Context &ctx, Goldilocks::Element &fi0, Goldilocks::Element &fi1, Goldilocks::Element &fi2, Goldilocks::Element &fi3, Goldilocks::Element &fi4, Goldilocks::Element &fi5, Goldilocks::Element &fi6, Goldilocks::Element &fi7, int32_t hashAddr)
{
    ...
    zkassert(ctx.rom.line[zkPC].hashS == 1);
                        ...
                        int32_t hashAddr)
{
    ...
    if (size > 32)
    {
        zklog.error("HashS_verify() Invalid size>32 for hashS 2: pols.D0[i]=" + fr.toString(ctx.pols.D0[i], 16));
        return ZKR_SM_MAIN_HASHS_SIZE_OUT_OF_RANGE;
    }
    ...
    {
        if (!fea2scalar(fr, a, ctx.pols.FREE0[i], ctx.pols.FREE1[i], ctx.pols.FREE2[i], ctx.pols.FREE3[i], ctx.pols.FREE4[i], ctx.pols.FREE5[i], ctx.pols.FREE6[i], ctx.pols.FREE7[i]))
        {
            zklog.error("HashS_verify() Failed calling fea2scalar(pols.FREE)");
