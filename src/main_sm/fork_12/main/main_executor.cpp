...
#include "main_sm/fork_12/main/rom_command.hpp"
#include "main_sm/fork_12/main/rom.hpp"
#include "main_sm/fork_12/main/context.hpp"
#include "main_sm/fork_12/main/eval_command.hpp"
#include "main_sm/fork_12/helpers/arith_helper.hpp"
#include "main_sm/fork_12/helpers/binary_helper.hpp"
#include "main_sm/fork_12/helpers/memory_helper.hpp"
#include "main_sm/fork_12/helpers/memalign_helper.hpp"
#include "main_sm/fork_12/helpers/storage_helper.hpp"
#include "main_sm/fork_12/helpers/keccak_helper.hpp"
#include "main_sm/fork_12/helpers/poseidon_helper.hpp"
#include "main_sm/fork_12/helpers/sha_helper.hpp"
    ...
    config(config)
{
    ...
    json romJson;
    file2json("src/main_sm/fork_12/scripts/rom_12.json", romJson);
    romBatch_12.load(fr, romJson);

    // Load diagnostic (unit test) ROM definition file
    if (config.loadDiagnosticRom)
    {
        file2json("src/main_sm/fork_12/scripts/rom_diagnostic.json", romJson);
        romDiagnostic.load(fr, romJson);
    }

#ifdef MULTI_ROM_TEST
    romJson.clear();
    file2json("src/main_sm/fork_12/scripts/rom_gas_limit_100000000.json", romJson);
    rom_gas_limit_100000000.load(fr, romJson);
    romJson.clear();
    file2json("src/main_sm/fork_12/scripts/rom_gas_limit_2147483647.json", romJson);
    rom_gas_limit_2147483647.load(fr, romJson);
    romJson.clear();
    file2json("src/main_sm/fork_12/scripts/rom_gas_limit_89128960.json", romJson);
    rom_gas_limit_89128960.load(fr, romJson);
#endif

    // Get labels
    finalizeExecutionLabel     = romBatch_12.getLabel(string("finalizeExecution"));
    checkAndSaveFromLabel      = romBatch_12.getLabel(string("checkAndSaveFrom"));
    ecrecoverStoreArgsLabel    = romBatch_12.getLabel(string("ecrecover_store_args"));
    ecrecoverEndLabel          = romBatch_12.getLabel(string("ecrecover_end"));
    checkFirstTxTypeLabel      = romBatch_12.getLabel(string("checkFirstTxType"));
    writeBlockInfoRootLabel    = romBatch_12.getLabel(string("writeBlockInfoRoot"));
    verifyMerkleProofEndLabel  = romBatch_12.getLabel(string("verifyMerkleProofEnd"));
    outOfCountersStepLabel     = romBatch_12.getLabel(string("outOfCountersStep"));
    outOfCountersArithLabel    = romBatch_12.getLabel(string("outOfCountersArith"));
    outOfCountersBinaryLabel   = romBatch_12.getLabel(string("outOfCountersBinary"));
    outOfCountersKeccakLabel   = romBatch_12.getLabel(string("outOfCountersKeccak"));
    outOfCountersSha256Label   = romBatch_12.getLabel(string("outOfCountersSha256"));
    outOfCountersMemalignLabel = romBatch_12.getLabel(string("outOfCountersMemalign"));
    outOfCountersPoseidonLabel = romBatch_12.getLabel(string("outOfCountersPoseidon"));
    outOfCountersPaddingLabel  = romBatch_12.getLabel(string("outOfCountersPadding"));
...
{
    TimerStart(MAIN_EXECUTOR_DESTRUCTOR_fork_12);
    ...
    TimerStopAndLog(MAIN_EXECUTOR_DESTRUCTOR_fork_12);
}
...
void MainExecutor::execute (ProverRequest &proverRequest, MainCommitPols &pols, MainExecRequired &required)
{
    ...
    if ( proverRequest.input.publicInputsExtended.publicInputs.forkID != 12)
    {
    ...
    // Get the proper rom
    Rom &rom = config.loadDiagnosticRom ? romDiagnostic : romBatch_12;
...
#define INVALID_LOG_ERROR_VALUE 999999999
    ...
    {
        romLine = (ctx.pZKPC != NULL) ? romBatch_12.line[*ctx.pZKPC].toString(fr) : "INVALID_ZKPC";
    }
