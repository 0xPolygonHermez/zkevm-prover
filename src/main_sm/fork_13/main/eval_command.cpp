...
void eval_eventLog(Context &ctx, const RomCommand &cmd, CommandResult &cr)
{
    ...
    zkassert(ctx.proverRequest.forkInfo.parentId == 13); // fork_13
    cr.zkResult = ((fork_13::FullTracer *)ctx.proverRequest.pFullTracer)->handleEvent(ctx, cmd);
...
zkresult eval_addReadWriteAddress (Context &ctx, const mpz_class value, const Goldilocks::Element (&key)[4])
{
    zkassert(ctx.proverRequest.pFullTracer != NULL);
    zkassert(ctx.proverRequest.forkInfo.parentId == 13); // fork_13
    return ((fork_13::FullTracer *)ctx.proverRequest.pFullTracer)->addReadWriteAddress(
...
void eval_MPdiv (Context &ctx, const RomCommand &cmd, CommandResult &cr)
{
    ...
    unordered_map<uint64_t, fork_13::Fea>::const_iterator it;
    mpz_class auxScalar;
