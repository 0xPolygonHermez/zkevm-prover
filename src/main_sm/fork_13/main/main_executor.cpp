...
#include "main_sm/fork_13/main/rom_line.hpp"
#include "main_sm/fork_13/main/rom_command.hpp"
#include "main_sm/fork_13/main/rom.hpp"
#include "main_sm/fork_13/main/context.hpp"
#include "main_sm/fork_13/main/eval_command.hpp"
#include "main_sm/fork_13/helpers/arith_helper.hpp"
#include "main_sm/fork_13/helpers/binary_helper.hpp"
#include "main_sm/fork_13/helpers/memory_helper.hpp"
#include "main_sm/fork_13/helpers/memalign_helper.hpp"
#include "main_sm/fork_13/helpers/storage_helper.hpp"
#include "main_sm/fork_13/helpers/keccak_helper.hpp"
#include "main_sm/fork_13/helpers/poseidon_helper.hpp"
#include "main_sm/fork_13/helpers/sha_helper.hpp"
    ...
    config(config)
{
    ...
    json romJson;
    file2json("src/main_sm/fork_13/scripts/rom_13.json", romJson);
    romBatch_13.load(fr, romJson);

    // Load diagnostic (unit test) ROM definition file
    if (config.loadDiagnosticRom)
    {
        file2json("src/main_sm/fork_13/scripts/rom_diagnostic.json", romJson);
        romDiagnostic.load(fr, romJson);
    }

#ifdef MULTI_ROM_TEST
    romJson.clear();
    file2json("src/main_sm/fork_13/scripts/rom_gas_limit_100000000.json", romJson);
    rom_gas_limit_100000000.load(fr, romJson);
    romJson.clear();
    file2json("src/main_sm/fork_13/scripts/rom_gas_limit_2147483647.json", romJson);
    rom_gas_limit_2147483647.load(fr, romJson);
    romJson.clear();
    file2json("src/main_sm/fork_13/scripts/rom_gas_limit_89128960.json", romJson);
    rom_gas_limit_89128960.load(fr, romJson);
#endif

    // Get labels
    finalizeExecutionLabel     = romBatch_13.getLabel(string("finalizeExecution")); // fork_13
    checkAndSaveFromLabel      = romBatch_13.getLabel(string("checkAndSaveFrom"));
    ecrecoverStoreArgsLabel    = romBatch_13.getLabel(string("ecrecover_store_args"));
    ecrecoverEndLabel          = romBatch_13.getLabel(string("ecrecover_end"));
    checkFirstTxTypeLabel      = romBatch_13.getLabel(string("checkFirstTxType"));
    writeBlockInfoRootLabel    = romBatch_13.getLabel(string("writeBlockInfoRoot"));
    verifyMerkleProofEndLabel  = romBatch_13.getLabel(string("verifyMerkleProofEnd"));
    outOfCountersStepLabel     = romBatch_13.getLabel(string("outOfCountersStep"));
    outOfCountersArithLabel    = romBatch_13.getLabel(string("outOfCountersArith"));
    outOfCountersBinaryLabel   = romBatch_13.getLabel(string("outOfCountersBinary"));
    outOfCountersKeccakLabel   = romBatch_13.getLabel(string("outOfCountersKeccak"));
    outOfCountersSha256Label   = romBatch_13.getLabel(string("outOfCountersSha256"));
    outOfCountersMemalignLabel = romBatch_13.getLabel(string("outOfCountersMemalign"));
    outOfCountersPoseidonLabel = romBatch_13.getLabel(string("outOfCountersPoseidon"));
    outOfCountersPaddingLabel  = romBatch_13.getLabel(string("outOfCountersPadding"));
...
{
    TimerStart(MAIN_EXECUTOR_DESTRUCTOR_fork_13);
    ...
    TimerStopAndLog(MAIN_EXECUTOR_DESTRUCTOR_fork_13);
}
...
void MainExecutor::execute (ProverRequest &proverRequest, MainCommitPols &pols, MainExecRequired &required)
{
    ...
    if ( proverRequest.input.publicInputsExtended.publicInputs.forkID != 13) // fork_13
    {
    ...
    // Get the proper rom
    Rom &rom = config.loadDiagnosticRom ? romDiagnostic : romBatch_13; // fork_13
...
#define INVALID_LOG_ERROR_VALUE 999999999
    ...
    {
        romLine = (ctx.pZKPC != NULL) ? romBatch_13.line[*ctx.pZKPC].toString(fr) : "INVALID_ZKPC"; // fork_13
    }
