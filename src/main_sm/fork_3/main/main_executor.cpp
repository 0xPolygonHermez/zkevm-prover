...
#include "main_sm/fork_3/main/rom_command.hpp"
...
#include "main_sm/fork_3/main/opcode_address.hpp"
    ...
    config(config)
{
    ...
    json romJson;
    file2json("src/main_sm/fork_3/scripts/rom.json", romJson);
...
void MainExecutor::execute (ProverRequest &proverRequest, MainCommitPols &pols, MainExecRequired &required)
{
    ...
    zkresult zkr = pHashDB->flush(proverRequest.uuid, proverRequest.pFullTracer->get_new_state_root(), proverRequest.flushId, proverRequest.lastSentFlushId);
    if (zkr != ZKR_SUCCESS)
    {
