...
void MainExecutor::execute (ProverRequest &proverRequest, MainCommitPols &pols, MainExecRequired &required)
{
    ...
    if (pStateDB == NULL)
    {
        zklog.error("MainExecutor::execute() failed calling StateDBClientFactory::createStateDBClient() uuid=" + proverRequest.uuid);
    ...
    for (step=0; step<N_Max; step++)
    {
        ...
        if (!fr.isZero(rom.line[zkPC].inFREE))
        {
            ...
            if (rom.line[zkPC].freeInTag.op == op_empty) {
                ...
                if (rom.line[zkPC].sRD == 1)
                {
                    ...
                    if (zkResult != ZKR_SUCCESS)
                    {
                        ...
                        logError(ctx, string("failed calling pStateDB->get() result=") + zkresult2string(zkResult));
                        StateDBClientFactory::freeStateDBClient(pStateDB);
                        return;
                    }
                    ...
                    if (bProcessBatch)
                    {
                        ...
                        {
                            proverRequest.result = zkResult;
                            logError(ctx, string("MainExecutor::Execute() failed calling eval_addReadWriteAddress() 1 result=") + zkresult2string(zkResult));
                            StateDBClientFactory::freeStateDBClient(pStateDB);
                            return;
                        }
                    }
                ...
                if (rom.line[zkPC].sWR == 1)
                {
                    ...
                    {
                        proverRequest.result = zkResult;
                        logError(ctx, string("Failed calling pStateDB->set() result=") + zkresult2string(zkResult));
                        StateDBClientFactory::freeStateDBClient(pStateDB);
                        return;
                    }
                ...
                if ( (rom.line[zkPC].hashK == 1) || (rom.line[zkPC].hashK1 == 1) )
                {
                    ...
                    if (rom.line[zkPC].hashK == 1)
                    {
                        ...
                        if (size>32)
                        {
                            ...
                            logError(ctx, "Invalid size>32 for hashK 1: pols.D0[i]=" + fr.toString(pols.D0[i], 16));
                            StateDBClientFactory::freeStateDBClient(pStateDB);
                            return;
                        }
                    }
                    ...
                    {
                        proverRequest.result = ZKR_SM_MAIN_HASHK;
                        logError(ctx, "hashK 1 invalid size of hash: pos=" + to_string(pos));
                        StateDBClientFactory::freeStateDBClient(pStateDB);
                        return;
                    }
            ...
            else
            {
                ...
                {
                    proverRequest.result = cr.zkResult;
                    logError(ctx, "Main exec failed calling evalCommand() result=" + to_string(proverRequest.result) + "=" + zkresult2string(proverRequest.result));
                    StateDBClientFactory::freeStateDBClient(pStateDB);
                    return;
                }
        ...
        if (rom.line[zkPC].sRD == 1)
        {
            ...
            {
                proverRequest.result = zkResult;
                logError(ctx, string("MainExecutor::Execute() failed calling pStateDB->get() result=") + zkresult2string(zkResult));
                StateDBClientFactory::freeStateDBClient(pStateDB);
                return;
            }
            incCounter = smtGetResult.proofHashCounter + 2;
            ...
            if (bProcessBatch)
            {
                ...
                {
                    proverRequest.result = zkResult;
                    logError(ctx, string("MainExecutor::Execute() failed calling eval_addReadWriteAddress() 3 result=") + zkresult2string(zkResult));
                    StateDBClientFactory::freeStateDBClient(pStateDB);
                    return;
                }
            }
        ...
        if ( (rom.line[zkPC].hashP == 1) || (rom.line[zkPC].hashP1 == 1) )
        {
            ...
            if (rom.line[zkPC].hashP == 1)
            {
                ...
                {
                    proverRequest.result = ZKR_SM_MAIN_HASHP;
                    logError(ctx, "Invalid size>32 for hashP 2: pols.D0[i]=" + fr.toString(pols.D0[i], 16));
                    StateDBClientFactory::freeStateDBClient(pStateDB);
                    return;
                }
            }
        ...
        if (rom.line[zkPC].hashPLen == 1)
        {
            ...
            if (!hashPIterator->second.digestCalled)
            {
                ...
                {
                    proverRequest.result = zkResult;
                    logError(ctx, string("Failed calling pStateDB->setProgram() result=") + zkresult2string(zkResult));
                    StateDBClientFactory::freeStateDBClient(pStateDB);
                    return;
                }
        ...
        if (rom.line[zkPC].hashPDigest == 1)
        {
            ...
            if (hashPIterator == ctx.hashP.end())
            {
                ...
                {
                    proverRequest.result = zkResult;
                    logError(ctx, string("Failed calling pStateDB->getProgram() result=") + zkresult2string(zkResult));
                    StateDBClientFactory::freeStateDBClient(pStateDB);
                    return;
                }
        ...
        if (rom.line[zkPC].bin == 1)
        {
            if (rom.line[zkPC].binOpcode == 0) // ADD
            {
                ...
                if (c != expectedC)
                {
                    ...
                    logError(ctx, "Binary ADD operation does not match");
                    StateDBClientFactory::freeStateDBClient(pStateDB);
                    return;
                }
            ...
            else if (rom.line[zkPC].binOpcode == 1) // SUB
            {
                ...
                {
                    proverRequest.result = ZKR_SM_MAIN_BINARY;
                    logError(ctx, "Binary SUB operation does not match");
                    StateDBClientFactory::freeStateDBClient(pStateDB);
                    return;
                }
            ...
            else if (rom.line[zkPC].binOpcode == 2) // LT
            {
                ...
                {
                    proverRequest.result = ZKR_SM_MAIN_BINARY;
                    logError(ctx, "Binary LT operation does not match");
                    StateDBClientFactory::freeStateDBClient(pStateDB);
                    return;
                }
            ...
            else if (rom.line[zkPC].binOpcode == 4) // EQ
            {
                ...
                {
                    proverRequest.result = ZKR_SM_MAIN_BINARY;
                    logError( ctx, "Binary EQ operation does not match");
                    StateDBClientFactory::freeStateDBClient(pStateDB);
                    return;
                }
            ...
            else if (rom.line[zkPC].binOpcode == 5) // AND
            {
                ...
                {
                    proverRequest.result = ZKR_SM_MAIN_BINARY;
                    logError(ctx, "Binary AND operation does not match");
                    StateDBClientFactory::freeStateDBClient(pStateDB);
                    return;
                }
            ...
            else if (rom.line[zkPC].binOpcode == 6) // OR
            {
                ...
                {
                    proverRequest.result = ZKR_SM_MAIN_BINARY;
                    logError(ctx, "Binary OR operation does not match");
                    StateDBClientFactory::freeStateDBClient(pStateDB);
                    return;
                }
            ...
            else if (rom.line[zkPC].binOpcode == 7) // XOR
            {
                ...
                {
                    proverRequest.result = ZKR_SM_MAIN_BINARY;
                    logError(ctx, "Binary XOR operation does not match");
                    StateDBClientFactory::freeStateDBClient(pStateDB);
                    return;
                }
        ...
        if (rom.line[zkPC].JMPN == 1)
        {
            ...
            {
                logError(ctx, "MainExecutor::execute() JMPN invalid S33 value op0=" + to_string(jmpnCondValue));
                exitProcess();
            }
        ...
        if ( (rom.line[zkPC].cmdAfter.size() > 0) && (step < (N_Max - 1)) )
        {
            ...
            for (uint64_t j=0; j<rom.line[zkPC].cmdAfter.size(); j++)
            {
                ...
                {
                    proverRequest.result = cr.zkResult;
                    logError(ctx, "Failed calling evalCommand() after");
                    StateDBClientFactory::freeStateDBClient(pStateDB);
                    return;
                }
            }
    ...
    else
    {
        ...
        if (zkr != ZKR_SUCCESS)
        {
            ...
            logError(ctx, string("Failed calling pStateDB->flush() result=") + zkresult2string(zkr));
            StateDBClientFactory::freeStateDBClient(pStateDB);
            return;
        }
        StateDBClientFactory::freeStateDBClient(pStateDB);
