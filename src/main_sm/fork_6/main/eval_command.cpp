...
void eval_eventLog(Context &ctx, const RomCommand &cmd, CommandResult &cr)
{
    ...
    zkassert(ctx.proverRequest.input.publicInputsExtended.publicInputs.forkID == 6); // fork_6
    cr.zkResult = ((fork_6::FullTracer *)ctx.proverRequest.pFullTracer)->handleEvent(ctx, cmd);
...
{
    zkassert(ctx.proverRequest.input.publicInputsExtended.publicInputs.forkID == 6); // fork_6
    return ((fork_6::FullTracer *)ctx.proverRequest.pFullTracer)->addReadWriteAddress(
