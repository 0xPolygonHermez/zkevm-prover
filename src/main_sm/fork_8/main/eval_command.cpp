...
void eval_eventLog(Context &ctx, const RomCommand &cmd, CommandResult &cr)
{
    ...
    zkassert(ctx.proverRequest.input.publicInputsExtended.publicInputs.forkID == 8); // fork_8
    cr.zkResult = ((fork_8::FullTracer *)ctx.proverRequest.pFullTracer)->handleEvent(ctx, cmd);
...
void eval_MPdiv (Context &ctx, const RomCommand &cmd, CommandResult &cr)
{
    ...
    unordered_map<uint64_t, fork_8::Fea>::const_iterator it;
    mpz_class auxScalar;
