...
#include "main_sm/fork_8/main_exec_c/context_c.hpp"
#include "main_sm/fork_8/main_exec_c/variables_c.hpp"
#include "main_sm/fork_8/main_exec_c/batch_decode.hpp"
...
void MainExecutorC::execute (ProverRequest &proverRequest)
{
    ...
    if (proverRequest.input.publicInputsExtended.publicInputs.forkID != 8) // fork_8
    {
    ...
    gettimeofday(&t, NULL);
#endif
    result = ((fork_8::FullTracer *)proverRequest.pFullTracer)->onStartBatch(ctxc);
    if (result != ZKR_SUCCESS)
    {
...
#pragma omp parallel for  num_threads(16) //TODO: Make this configurable
    ...
    for (ctxc.tx=0; ctxc.tx<ctxc.batch.tx.size(); ctxc.tx++)
    {
        ...
        gettimeofday(&t, NULL);
#endif
        result = ((fork_8::FullTracer *)proverRequest.pFullTracer)->onProcessTx(ctxc);
        if (result != ZKR_SUCCESS)
        {
        ...
        gettimeofday(&t, NULL);
#endif
        result = ((fork_8::FullTracer *)proverRequest.pFullTracer)->onFinishTx(ctxc);
        if (result != ZKR_SUCCESS)
        {
    ...
    gettimeofday(&t, NULL);
#endif
    result = ((fork_8::FullTracer *)proverRequest.pFullTracer)->onFinishBatch(ctxc);
    if (result != ZKR_SUCCESS)
    {
