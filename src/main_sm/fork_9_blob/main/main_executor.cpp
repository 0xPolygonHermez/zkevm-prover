...
#include "main_sm/fork_9_blob/main/rom_line.hpp"
    ...
    config(config)
{
    ...
    json romJson;
    file2json("src/main_sm/fork_9_blob/scripts/rom.json", romJson);
    romBatch.load(fr, romJson);

    // Load Collection (unit test) ROM definition file
    file2json("src/main_sm/fork_9_blob/scripts/rom_collection.json", romJson);
    romCollection.load(fr, romJson);

#ifdef MULTI_ROM_TEST
    romJson.clear();
    file2json("src/main_sm/fork_9_blob/scripts/rom_gas_limit_100000000.json", romJson);
    rom_gas_limit_100000000.load(fr, romJson);
    romJson.clear();
    file2json("src/main_sm/fork_9_blob/scripts/rom_gas_limit_2147483647.json", romJson);
    rom_gas_limit_2147483647.load(fr, romJson);
    romJson.clear();
    file2json("src/main_sm/fork_9_blob/scripts/rom_gas_limit_89128960.json", romJson);
...
{
    TimerStart(MAIN_EXECUTOR_DESTRUCTOR_fork_9_blob);

    HashDBClientFactory::freeHashDBClient(pHashDB);

    TimerStopAndLog(MAIN_EXECUTOR_DESTRUCTOR_fork_9_blob);
}
