...
void Prover::prove(ProverRequest *pProverRequest)
{
    ...
    if (pProverRequest->result == ZKR_SUCCESS)
    {
        ...
        cmPols.Main.FREE0[0] = Goldilocks::fromU64(2043100198);
        cmPols.Main.FREE0[1] = Goldilocks::fromU64(2909753411);
        cmPols.Main.FREE0[2] = Goldilocks::fromU64(2146825699);
        cmPols.Main.FREE0[3] = Goldilocks::fromU64(3866023039);
        cmPols.Main.FREE0[4] = Goldilocks::fromU64(1719628537);
        cmPols.Main.FREE0[5] = Goldilocks::fromU64(3739677152);
        cmPols.Main.FREE0[6] = Goldilocks::fromU64(1596594856);
        cmPols.Main.FREE0[7] = Goldilocks::fromU64(3497182697);
...
#define starkFile "zkevm.prove.json"
        ...
        zkin.clear();
        std::ifstream zkinStream(zkinFile);
        if (!zkinStream.good())
        {
...
#pragma omp parallel for
        ...
        // zkin["publics"] = publicJson;
        ofstream ofzkin("zkevm.c12.zkin.proof.json");
        ofzkin << setw(4) << zkinC12.dump() << endl;
        ofzkin.close();

        delete (tmp);
        free(pAddressC12);
#else
        nlohmann::json zkinC12;
        TimerStart(PROVER_INJECT_ZKIN_JSON);
        zkinC12.clear();
        std::ifstream zkinStream("zkevm.c12.zkin.proof.json");
        if (!zkinStream.good())
        {
            cerr << "Error: failed loading zkin.json file " << endl;
            exitProcess();
        ...
        loadJsonImplC12(ctxC12, zkinC12);
