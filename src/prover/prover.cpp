    ...
    try
    {
        if (config.generateProof())
        {
            ...
            if(!externalAllocated) {
                ...
                string zkevmCHelpers = config.zkevmGenericCHelpers;
                string c12aCHelpers = config.c12aGenericCHelpers;
                string recursive1CHelpers = config.recursive1GenericCHelpers;
...
void Prover::genStarkProof(PROVER_FORK_NAMESPACE::CommitPols &cmPols, uint64_t lastN, ProverRequest *pProverRequest)
{
    ...
    nlohmann::ordered_json jProofc12a = fproofC12a.proof.proof2json();
    ...
    nlohmann::ordered_json jProofRecursive1 = fproofRecursive1.proof.proof2json();
...
void Prover::genAggregatedProof(ProverRequest *pProverRequest)
{
    ...
    json zkinInputRecursive2 = joinzkin(pProverRequest->aggregatedProofInput1, pProverRequest->aggregatedProofInput2, verKey, starksRecursive1->starkInfo);
    json recursive2Verkey;
    file2json(config.recursive2Verkey, recursive2Verkey);
    ...
    // Save the proof & zkinproof
    nlohmann::ordered_json jProofRecursive2 = fproofRecursive2.proof.proof2json();
...
void Prover::genFinalProof(ProverRequest *pProverRequest)
{
    ...
    // Save the proof & zkinproof
    nlohmann::ordered_json jProofRecursiveF = fproofRecursiveF.proof.proof2json();
