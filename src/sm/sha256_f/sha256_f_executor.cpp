...
void Sha256FExecutor::execute(const vector<vector<Goldilocks::Element>> &input, Sha256FCommitPols &pols)
{
    #if 0
...
#pragma omp parallel for
    ...
    zklog.info("KeccakFExecutor successfully processed " + to_string(numberOfSlots) + " Keccak-F actions (" + to_string((double(input.size()) * SHA256GateConfig.slotSize * 100) / N) + "%)");
    #endif
}

void Sha256FExecutor::setPol(CommitPol (&pol)[4], uint64_t index, uint64_t value)
{
...
uint64_t Sha256FExecutor::getPol(CommitPol (&pol)[4], uint64_t index)
{
