...
#define handle_error(msg) \
  ...
  void getCommitedPols(CommitPolsStarks *commitPols, const std::string blobInnerVerifier, const std::string execFile, nlohmann::json &zkin, uint64_t N, uint64_t nCols)
  {
    ...
    //-------------------------------------------
    TimerStart(CIRCOM_LOAD_CIRCUIT_BLOB_INNER_PROOF);
    Circom_Circuit *circuit = loadCircuit(blobInnerVerifier);
    TimerStopAndLog(CIRCOM_LOAD_CIRCUIT_BLOB_INNER_PROOF);
    TimerStart(CIRCOM_LOAD_JSON_BLOB_INNER_PROOF);
    ...
    TimerStopAndLog(CIRCOM_LOAD_JSON_BLOB_INNER_PROOF);

    //-------------------------------------------
    // Compute witness and commited pols
    //-------------------------------------------
    TimerStart(STARK_WITNESS_AND_COMMITED_POLS_BLOB_INNER_PROOF);
    ...
    delete[] tmp;
    freeCircuit(circuit);
    TimerStopAndLog(STARK_WITNESS_AND_COMMITED_POLS_BLOB_INNER_PROOF);
  }
