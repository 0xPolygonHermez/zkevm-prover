...
#define handle_error(msg) \
  ...
  void getCommitedPols(CommitPolsStarks *commitPols, const std::string blobInnerRecursive1Circuit, const std::string execFile, nlohmann::json &zkin, uint64_t N, uint64_t nCols)
  {
    ...
    //-------------------------------------------
    TimerStart(CIRCOM_LOAD_CIRCUIT_BLOB_INNER_RECURSIVE1_PROOF);
    Circom_Circuit *circuit = loadCircuit(blobInnerRecursive1Circuit);
    TimerStopAndLog(CIRCOM_LOAD_CIRCUIT_BLOB_INNER_RECURSIVE1_PROOF);
    TimerStart(CIRCOM_LOAD_JSON_BLOB_INNER_RECURSIVE1_PROOF);
    ...
    TimerStopAndLog(CIRCOM_LOAD_JSON_BLOB_INNER_RECURSIVE1_PROOF);

    //-------------------------------------------
    // Compute witness and commited pols
    //-------------------------------------------
    TimerStart(STARK_WITNESS_AND_COMMITED_POLS_BLOB_INNER_RECURSIVE1_PROOF);
    ...
    delete[] tmp;
    freeCircuit(circuit);
    TimerStopAndLog(STARK_WITNESS_AND_COMMITED_POLS_BLOB_INNER_RECURSIVE1_PROOF);
  }
