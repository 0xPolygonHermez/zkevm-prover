...
#define handle_error(msg) \
  ...
  void getCommitedPols(CommitPolsStarks *commitPols, const std::string blobOuterCircuit, const std::string execFile, nlohmann::json &zkin, uint64_t N, uint64_t nCols)
  {
    ...
    //-------------------------------------------
    TimerStart(CIRCOM_LOAD_CIRCUIT_BLOB_OUTER_PROOF);
    Circom_Circuit *circuit = loadCircuit(blobOuterCircuit);
    TimerStopAndLog(CIRCOM_LOAD_CIRCUIT_BLOB_OUTER_PROOF);
    TimerStart(CIRCOM_LOAD_JSON_BLOB_OUTER_PROOF);
    ...
    TimerStopAndLog(CIRCOM_LOAD_JSON_BLOB_OUTER_PROOF);

    //-------------------------------------------
    // Compute witness and commited pols
    //-------------------------------------------
    TimerStart(STARK_WITNESS_AND_COMMITED_POLS_BLOB_OUTER_PROOF);
    ...
    delete[] tmp;
    freeCircuit(circuit);
    TimerStopAndLog(STARK_WITNESS_AND_COMMITED_POLS_BLOB_OUTER_PROOF);
  }
