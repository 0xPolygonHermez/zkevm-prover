...
#include "circom.blob_outer_recursive2.hpp"
...
#define handle_error(msg) \
  ...
  void getCommitedPols(CommitPolsStarks *commitPols, const std::string blobOuterRecursive2Circuit, const std::string execFile, nlohmann::json &zkin, uint64_t N, uint64_t nCols)
  {
    //-------------------------------------------
    // Verifier stark proof
    //-------------------------------------------
    TimerStart(CIRCOM_LOAD_CIRCUIT_BLOB_OUTER_RECURSIVE2_PROOF);
    Circom_Circuit *circuit = loadCircuit(blobOuterRecursive2Circuit);
    TimerStopAndLog(CIRCOM_LOAD_CIRCUIT_BLOB_OUTER_RECURSIVE2_PROOF);
    TimerStart(CIRCOM_LOAD_JSON_BLOB_OUTER_RECURSIVE2_PROOF);
    ...
    TimerStopAndLog(CIRCOM_LOAD_JSON_BLOB_OUTER_RECURSIVE2_PROOF);

    //-------------------------------------------
    // Compute witness and commited pols
    //-------------------------------------------
    TimerStart(STARK_WITNESS_AND_COMMITED_POLS_BLOB_OUTER_RECURSIVE2_PROOF);
    ...
    delete[] tmp;
    freeCircuit(circuit);
    TimerStopAndLog(STARK_WITNESS_AND_COMMITED_POLS_BLOB_OUTER_RECURSIVE2_PROOF);
  }
