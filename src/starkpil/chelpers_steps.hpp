            ...
            for (uint64_t kk = 0; kk < parserParams.nOps; ++kk) {
                ...
                case 45: {
                    ...
                    if(needModule) {
                        ...
                        offsetsDest[2] = stepOffset + ((nextStrideOffset + 2) % domainSize) * nColsSteps[args[i_args]];
                        offsetsDest[3] = stepOffset + ((nextStrideOffset + 3) % domainSize) * nColsSteps[args[i_args]];
                        Goldilocks3::store_avx(&params.pols[0], offsetsDest, &bufferT_[buffTOffsetsSteps_[args[i_args]] + 2 * args[i_args + 1] + args[i_args + 2]], 2);
                    } else {
                        Goldilocks3::store_avx(&params.pols[offsetsSteps[args[i_args]] + args[i_args + 1] + (i + nextStride * args[i_args + 2]) * nColsSteps[args[i_args]]], nColsSteps[args[i_args]], &bufferT_[buffTOffsetsSteps_[args[i_args]] + 2 * args[i_args + 1] + args[i_args + 2]], 2);
                    }
                    i_args += 6;
                    break;
            }
