#ifndef CHELPERS_STEPS_BATCH_HPP
#define CHELPERS_STEPS_BATCH_HPP
#include "chelpers.hpp"
#include "chelpers_steps.hpp"
#include "steps.hpp"

class CHelpersStepsBatch : public CHelpersSteps {
public:
    virtual void storePolinomial(Goldilocks::Element *pols, Goldilocks::Element *bufferT, uint64_t* nColsSteps, uint64_t *offsetsSteps, uint64_t *buffTOffsetsSteps, uint64_t *nextStrides, uint64_t nOpenings, uint64_t domainSize, bool domainExtended, uint64_t nStages, bool needModule, uint64_t row, uint64_t stage, uint64_t stagePos, uint64_t openingPointIndex, uint64_t dim) {
        if(needModule) {
    ...
    #pragma omp parallel for
        for (uint64_t i = 0; i < domainSize; i+= nrowsBatch) {
            ...
            for (uint64_t kk = 0; kk < parserParams.nOps; ++kk) {
                ...
                Goldilocks::copy_batch(&tmp1[args[i_args]], &bufferT_[buffTOffsetsSteps_[args[i_args + 1]] + 2 * args[i_args + 2] + args[i_args + 3]]);
                    i_args += 4;
                    break;
            }
                case 16: {
                ...
                case 20: {
                    // COPY tmp1 to tmp1
                Goldilocks::copy_batch(&tmp1[args[i_args]], &tmp1[args[i_args + 1]]);
                    i_args += 2;
                    break;
            }
                case 21: {
                ...
                case 24: {
                    // COPY public to tmp1
                Goldilocks::copy_batch(&tmp1[args[i_args]], params.publicInputs[args[i_args + 1]]);
                    i_args += 2;
                    break;
            }
                case 25: {
                ...
                case 27: {
                    // COPY x to tmp1
                Goldilocks::copy_batch(&tmp1[args[i_args]], x[i]);
                    i_args += 1;
                    break;
            }
                case 28: {
                    // COPY number to tmp1
                Goldilocks::copy_batch(&tmp1[args[i_args]], numbers_[args[i_args + 1]]);
                    i_args += 2;
                    break;
            }
                case 29: {
                ...
                Goldilocks3::copy_batch(&tmp3[args[i_args] * FIELD_EXTENSION], &bufferT_[buffTOffsetsSteps_[args[i_args + 1]] + 2 * args[i_args + 2] + args[i_args + 3]]);
                    i_args += 4;
                    break;
            }
                case 72: {
                ...
                case 76: {
                    // COPY tmp3 to tmp3
                Goldilocks3::copy_batch(&tmp3[args[i_args] * FIELD_EXTENSION], &tmp3[args[i_args + 1] * FIELD_EXTENSION]);
                    i_args += 2;
                    break;
            }
                case 77: {
                ...
                case 82: {
                    // COPY eval to tmp3
                Goldilocks3::copy_batch(&tmp3[args[i_args] * FIELD_EXTENSION], &params.evals[args[i_args + 1]]);
                    i_args += 2;
                    break;
            }
                case 83: {
                ...
                case 91: {
                    // COPY tmp3 to f
                    Goldilocks3::copy_batch(tmp3_, tmp3[args[i_args]]);
                    Goldilocks3::store_batch(&params.f_2ns[i*3], uint64_t(3), tmp3_);
                    i_args += 1;
                    break;
            }
                    default: {
