...
void FRIProveC12::queryPol(FRIProofC12 &fproof, MerkleTreeBN128 **trees, uint64_t idx, uint64_t treeIdx)
{
    ...
    for (uint i = 0; i < 5; i++)
    {
        ...
        if(buff==NULL){
            std::cout << "queryPol: malloc failed" << std::endl;
            exitProcess();            
        }
...
void FRIProveC12::queryPol(FRIProofC12 &fproof, MerkleTreeBN128 *tree, uint64_t idx, uint64_t treeIdx)
{
    ...
    if(buff==NULL){
        std::cout << "queryPol(FRIProofC12 &fproof, MerkleTreeBN128 *tree, uint64_t idx, uint64_t treeIdx): malloc failed" << std::endl;
        exitProcess();
    }
