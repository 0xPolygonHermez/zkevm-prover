...
void getPolynomial(SetupCtx& setupCtx, StepsParams& params, Goldilocks::Element *dest, bool committed, uint64_t idPol, bool domainExtended) {
...
void setPolynomial(SetupCtx& setupCtx, StepsParams& params, Goldilocks::Element *values, uint64_t idPol, bool domainExtended) {
    ...
    for(uint64_t i = 0; i < setupCtx.starkInfo.cmPolsMap.size(); ++i) {
        ...
        if(cmPol.name == name) {
            ...
            if(returnValues) {
                ...
                getPolynomial(setupCtx, params, hintFieldInfo.values, true, i, false);
            }
    ...
    for(uint64_t i = 0; i < setupCtx.starkInfo.constPolsMap.size(); ++i) {
        ...
        if(constPol.name == name) {
            ...
            if(returnValues) {
                ...
                getPolynomial(setupCtx, params, hintFieldInfo.values, false, i, false);
            }
            return hintFieldInfo;
        } 
    }
...
HintFieldInfo getHintField(SetupCtx& setupCtx, StepsParams& params, uint64_t hintId, std::string hintFieldName, bool dest, bool inverse, bool print_expression) {
    ...
    if(hintField->operand == opType::cm) {
        ...
        if(!dest) {
            getPolynomial(setupCtx, params, hintFieldInfo.values, true, hintField->id, false);
            if(inverse) {
        ...
        cout << endl;
        getPolynomial(setupCtx, params, hintFieldInfo.values, false, hintField->id, false);
        if(inverse) {
            zklog.error("Inverse not supported still for polynomials");
            exitProcess();
        }
...
uint64_t setHintField(SetupCtx& setupCtx, StepsParams& params, Goldilocks::Element* values, uint64_t hintId, std::string hintFieldName) {
    ...
    if(hintField->operand == opType::cm) {
        setPolynomial(setupCtx, params, values, hintField->id, false);
    } else if(hintField->operand == opType::subproofvalue) {
