...
void ProveFRI::polMulAxi(Polinomial &pol, Goldilocks::Element init, Goldilocks::Element acc)
{
...
void ProveFRI::evalPol(Polinomial &res, uint64_t res_idx, Polinomial &p, Polinomial &x)
{
    if (p.degree() == 0)
    {
...
void ProveFRI::queryPol(FriProof &fproof, Goldilocks::Element *trees[5], uint64_t idx, uint64_t treeIdx)
{
...
void ProveFRI::queryPol(FriProof &fproof, Goldilocks::Element *tree, uint64_t idx, uint64_t treeIdx)
{
    vector<MerkleProof> vMkProof;
