...
StarkRecursive1::StarkRecursive1(const Config &config) : config(config),
...

{
    ...
    TimerStart(LOAD_RECURSIVE_1_CONST_POLS_TO_MEMORY);
    pConstPolsAddress = NULL;
    if (config.constPolsRecursive1File.size() == 0)
    {
        cerr << "Error: StarkRecursive1::StarkRecursive1() received an empty config.constPolsRecursive1File" << endl;
        exit(-1);
    }

    if (config.mapConstPolsFile)
    {
        pConstPolsAddress = mapFile(config.constPolsRecursive1File, ConstantPolsRecursive1::pilSize(), false);
        cout << "StarkRecursive1::StarkRecursive1() successfully mapped " << ConstantPolsRecursive1::pilSize() << " bytes from constant file " << config.constPolsRecursive1File << endl;
    }
    else
    {
        pConstPolsAddress = copyFile(config.constPolsRecursive1File, ConstantPolsRecursive1::pilSize());
        cout << "StarkRecursive1::StarkRecursive1() successfully copied " << ConstantPolsRecursive1::pilSize() << " bytes from constant file " << config.constPolsRecursive1File << endl;
    }
    ...
    TimerStart(LOAD_RECURSIVE_1_CONST_TREE_TO_MEMORY);
    pConstTreeAddress = NULL;
    if (config.constantsTreeRecursive1File.size() == 0)
    {
        cerr << "Error: StarkRecursive1::StarkRecursive1() received an empty config.constantsTreeRecursive1File" << endl;
        exit(-1);
    }

    if (config.mapConstantsTreeFile)
    {
        pConstTreeAddress = mapFile(config.constantsTreeRecursive1File, starkInfo.getConstTreeSizeInBytes(), false);
        cout << "StarkRecursive1::StarkRecursive1() successfully mapped " << starkInfo.getConstTreeSizeInBytes() << " bytes from constant tree file " << config.constantsTreeRecursive1File << endl;
    }
    else
    {
        pConstTreeAddress = copyFile(config.constantsTreeRecursive1File, starkInfo.getConstTreeSizeInBytes());
        cout << "StarkRecursive1::StarkRecursive1() successfully copied " << starkInfo.getConstTreeSizeInBytes() << " bytes from constant file " << config.constantsTreeRecursive1File << endl;
    }
    TimerStopAndLog(LOAD_RECURSIVE_1_CONST_TREE_TO_MEMORY);
...
void StarkRecursive1::genProof(void *pAddress, FRIProof &proof, Goldilocks::Element publicInputs[43])
{
    ...
    ///////////
    TimerStart(STARK_RECURSIVE_1_STEP_1);
    ...
    TimerStopAndLog(STARK_RECURSIVE_1_STEP_1_MERKLETREE);
    TimerStopAndLog(STARK_RECURSIVE_1_STEP_1_LDE_AND_MERKLETREE);
    TimerStopAndLog(STARK_RECURSIVE_1_STEP_1);

    ///////////
    // 2.- Caluculate plookups h1 and h2
    ///////////
    TimerStart(STARK_RECURSIVE_1_STEP_2);
    ...
    TimerStart(STARK_RECURSIVE_1_STEP_2_CALCULATE_EXPS);
...
#pragma omp parallel for
    ...
    TimerStopAndLog(STARK_RECURSIVE_1_STEP_2_CALCULATE_EXPS);
    TimerStart(STARK_RECURSIVE_1_STEP_2_CALCULATEH1H2);
#pragma omp parallel for
    ...
    TimerStopAndLog(STARK_RECURSIVE_1_STEP_2_CALCULATEH1H2);

    TimerStart(STARK_RECURSIVE_1_STEP_2_LDE_AND_MERKLETREE);
    ...
    TimerStopAndLog(STARK_RECURSIVE_1_STEP_2_MERKLETREE);
    TimerStopAndLog(STARK_RECURSIVE_1_STEP_2_LDE_AND_MERKLETREE);
    TimerStopAndLog(STARK_RECURSIVE_1_STEP_2);

    ///////////
    // 3.- Compute Z polynomials
    ///////////

    TimerStart(STARK_RECURSIVE_1_STEP_3);
    ...
    TimerStart(STARK_RECURSIVE_1_STEP_3_CALCULATE_EXPS);
...
#pragma omp parallel for
    ...
    TimerStopAndLog(STARK_RECURSIVE_1_STEP_3_CALCULATE_EXPS);
    TimerStart(STARK_RECURSIVE_1_STEP_3_CALCULATE_Z);
    ...
    TimerStopAndLog(STARK_RECURSIVE_1_STEP_3_CALCULATE_Z);

    TimerStart(STARK_RECURSIVE_1_STEP_3_LDE_AND_MERKLETREE);
    TimerStart(STARK_RECURSIVE_1_STEP_3_LDE);
    ...
    TimerStopAndLog(STARK_RECURSIVE_1_STEP_3_MERKLETREE);
    TimerStopAndLog(STARK_RECURSIVE_1_STEP_3_LDE_AND_MERKLETREE);
    TimerStopAndLog(STARK_RECURSIVE_1_STEP_3);

    ///////////
    // 4. Compute C Polynomial
    ///////////

    TimerStart(STARK_RECURSIVE_1_STEP_4);
    TimerStart(STARK_RECURSIVE_1_STEP_4_CALCULATE_EXPS);
...
#pragma omp parallel for
    ...
    TimerStopAndLog(STARK_RECURSIVE_1_STEP_4_CALCULATE_EXPS);
...
#pragma omp parallel for
    ...
    TimerStopAndLog(STARK_RECURSIVE_1_STEP_4_MERKLETREE);
    TimerStopAndLog(STARK_RECURSIVE_1_STEP_4);

    ///////////
    // 5. Compute FRI Polynomial
    ///////////
    TimerStart(STARK_RECURSIVE_1_STEP_5);
    TimerStart(STARK_RECURSIVE_1_STEP_5_LEv_LpEv);
    ...
    TimerStopAndLog(STARK_RECURSIVE_1_STEP_5_LEv_LpEv);

    TimerStart(STARK_RECURSIVE_1_STEP_5_EVMAP);
...
#pragma omp for
    ...
    TimerStopAndLog(STARK_RECURSIVE_1_STEP_5_EVMAP);
    TimerStart(STARK_RECURSIVE_1_STEP_5_XDIVXSUB);
    ...
    *x1[0] = Goldilocks::shift();      // [49,0,0]
    ...
    TimerStopAndLog(STARK_RECURSIVE_1_STEP_5_XDIVXSUB);
    TimerStart(STARK_RECURSIVE_1_STEP_5_CALCULATE_EXPS);
...
#pragma omp parallel for
    ...
    TimerStopAndLog(STARK_RECURSIVE_1_STEP_5_CALCULATE_EXPS);
    TimerStopAndLog(STARK_RECURSIVE_1_STEP_5);
    TimerStart(STARK_RECURSIVE_1_STEP_FRI);
