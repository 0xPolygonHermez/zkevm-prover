...
#define handle_error(msg) \
  ...
  void getCommitedPols(CommitPolsStarks *commitPols, const std::string recursive2Circuit, const std::string execFile, nlohmann::json &zkin, uint64_t N, uint64_t nCols)
  {
    ...
    //-------------------------------------------
    TimerStart(CIRCOM_LOAD_CIRCUIT_BATCH_RECURSIVE_2_PROOF);
    Circom_Circuit *circuit = loadCircuit(recursive2Circuit);
    TimerStopAndLog(CIRCOM_LOAD_CIRCUIT_BATCH_RECURSIVE_2_PROOF);
    TimerStart(CIRCOM_LOAD_JSON_BATCH_RECURSIVE_2_PROOF);
    ...
    TimerStopAndLog(CIRCOM_LOAD_JSON_BATCH_RECURSIVE_2_PROOF);
    //-------------------------------------------
    // Compute witness and commited pols
    //-------------------------------------------
    TimerStart(STARK_WITNESS_AND_COMMITED_POLS_BATCH_RECURSIVE_2_PROOF);
    ...
    delete[] tmp;
    freeCircuit(circuit);
    TimerStopAndLog(STARK_WITNESS_AND_COMMITED_POLS_BATCH_RECURSIVE_2_PROOF);
  }
}
