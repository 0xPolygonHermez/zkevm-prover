...
StarkC12Mock::StarkC12Mock(const Config &config) : config(config),
...

{
    ...
    {
        cerr << "Error: StarkC12Mock::StarkC12Mock() received an empty config.constPolsC12File" << endl;
        exit(-1);
    }

    if (config.mapConstPolsFile)
    {
        pConstPolsAddress = mapFile(config.constPolsC12File, ConstantPolsBasicC12::pilSize(), false);
        cout << "StarkC12Mock::StarkC12Mock() successfully mapped " << ConstantPolsBasicC12::pilSize() << " bytes from constant file " << config.constPolsC12File << endl;
    }
    else
    {
        pConstPolsAddress = copyFile(config.constPolsC12File, ConstantPolsBasicC12::pilSize());
        cout << "StarkC12Mock::StarkC12Mock() successfully copied " << ConstantPolsBasicC12::pilSize() << " bytes from constant file " << config.constPolsC12File << endl;
    }
    ...
    {
        cerr << "Error: StarkC12Mock::StarkC12Mock() received an empty config.constantsTreeC12File" << endl;
        exit(-1);
    }
    ...
    if (config.mapConstantsTreeFile)
    {
        ...
        cout << "StarkC12Mock::StarkC12Mock() successfully mapped " << getTreeSize((1 << starkInfo.starkStruct.nBitsExt), starkInfo.nConstants) << " bytes from constant tree file " << config.constantsTreeC12File << endl;
    }
    else
    {
        pConstTreeAddress = copyFile(config.constantsTreeC12File, getTreeSize((1 << starkInfo.starkStruct.nBitsExt), starkInfo.nConstants));
        cout << "StarkC12Mock::StarkC12Mock() successfully copied " << getTreeSize((1 << starkInfo.starkStruct.nBitsExt), starkInfo.nConstants) << " bytes from constant file " << config.constantsTreeC12File << endl;
    }
...
StarkC12Mock::~StarkC12Mock()
{
    if (!config.generateProof())
        return;

    /*    cout << "StarkC12Mock::~StarkC12Mock()" << endl;
    ...
    else
    {
        ...
        // cout << "StarkC12Mock::~StarkC12Mock() free(pConstPolsAddress)" << endl;
    }
    ...
    else
    {
        ...
        // cout << "StarkC12Mock::~StarkC12Mock() free(pConstTreeAddress)" << endl;
    }
...
void StarkC12Mock::genProof(void *pAddress, FRIProofC12 &proof, Goldilocks::Element publicInputs[8])
{
    CommitPolsBasicC12 cmPols(pAddress, starkInfo.mapDeg.section[eSection::cm1_n]);
