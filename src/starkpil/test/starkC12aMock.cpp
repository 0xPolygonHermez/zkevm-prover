...
StarkC12aMock::StarkC12aMock(const Config &config) : config(config),
...

{
    ...
    {
        cerr << "Error: StarkC12aMock::StarkC12aMock() received an empty config.constPolsC12aFile" << endl;
        exit(-1);
    }

    if (config.mapConstPolsFile)
    {
        pConstPolsAddress = mapFile(config.constPolsC12aFile, ConstantPolsBasicC12::pilSize(), false);
        cout << "StarkC12aMock::StarkC12aMock() successfully mapped " << ConstantPolsBasicC12::pilSize() << " bytes from constant file " << config.constPolsC12aFile << endl;
    }
    else
    {
        pConstPolsAddress = copyFile(config.constPolsC12aFile, ConstantPolsBasicC12::pilSize());
        cout << "StarkC12aMock::StarkC12aMock() successfully copied " << ConstantPolsBasicC12::pilSize() << " bytes from constant file " << config.constPolsC12aFile << endl;
    }
    pConstPols = new ConstantPolsBasicC12(pConstPolsAddress, ConstantPolsBasicC12::pilDegree());
    ...
    {
        cerr << "Error: StarkC12aMock::StarkC12aMock() received an empty config.constantsTreeC12aFile" << endl;
        exit(-1);
    }
    ...
    if (config.mapConstantsTreeFile)
    {
        ...
        cout << "StarkC12aMock::StarkC12aMock() successfully mapped " << starkInfo.getConstTreeSizeInBytes() << " bytes from constant tree file " << config.constantsTreeC12aFile << endl;
    }
    else
    {
        pConstTreeAddress = copyFile(config.constantsTreeC12aFile, starkInfo.getConstTreeSizeInBytes());
        cout << "StarkC12aMock::StarkC12aMock() successfully copied " << starkInfo.getConstTreeSizeInBytes() << " bytes from constant file " << config.constantsTreeC12aFile << endl;
    }
    ...
    pConstPols2ns = new ConstantPolsBasicC12(pConstPolsAddress2ns, (1 << starkInfo.starkStruct.nBitsExt));
...
StarkC12aMock::~StarkC12aMock()
{
    if (!config.generateProof())
        return;

    /*    cout << "StarkC12aMock::~StarkC12aMock()" << endl;
    ...
    if (config.mapConstPolsFile)
    {
        unmapFile(pConstPolsAddress, ConstantPolsBasicC12::pilSize());
    }
    else
    {
        free(pConstPolsAddress);
        // cout << "StarkC12aMock::~StarkC12aMock() free(pConstPolsAddress)" << endl;
    }
    ...
    else
    {
        ...
        // cout << "StarkC12aMock::~StarkC12aMock() free(pConstTreeAddress)" << endl;
    }
...
#pragma omp parallel for
    ...
    if (starkInfo.puCtx.size() != 0 || starkInfo.peCtx.size() != 0 || starkInfo.ciCtx.size() != 0)
    {
        Polinomial aux;
        if (starkInfo.puCtx.size() != 0)
        {
            aux = starkInfo.getPolinomial(mem, starkInfo.exps_n[starkInfo.puCtx[0].numId]);
        }
        else if (starkInfo.peCtx.size() != 0)
        {
            aux = starkInfo.getPolinomial(mem, starkInfo.exps_n[starkInfo.peCtx[0].numId]);
        }
        else if (starkInfo.ciCtx.size() != 0)
        {
            aux = starkInfo.getPolinomial(mem, starkInfo.exps_n[starkInfo.ciCtx[0].numId]);
        }
...
#pragma omp parallel for
    ...
    std::memcpy(&proof.proofs.root1[0], root1.address(), sizeof(RawFr::Element));
    std::memcpy(&proof.proofs.root2[0], root2.address(), sizeof(RawFr::Element));
    std::memcpy(&proof.proofs.root3[0], root3.address(), sizeof(RawFr::Element));
    std::memcpy(&proof.proofs.root4[0], root4.address(), sizeof(RawFr::Element));
