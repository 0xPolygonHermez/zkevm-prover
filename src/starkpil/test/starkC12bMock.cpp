...

{
    ...
    pConstPols2ns = new ConstantPolsBasicC12b(pConstPolsAddress2ns, (1 << starkInfo.starkStruct.nBitsExt));
...
StarkC12bMock::~StarkC12bMock()
{
    if (!config.generateProof())
        return;

    /*    cout << "StarkC12bMock::~StarkC12bMock()" << endl;
    ...
    if (config.mapConstPolsFile)
    {
        unmapFile(pConstPolsAddress, ConstantPolsBasicC12b::pilSize());
    }
    else
    {
        free(pConstPolsAddress);
        // cout << "StarkC12bMock::~StarkC12bMock() free(pConstPolsAddress)" << endl;
    }
    ...
    else
    {
        ...
        // cout << "StarkC12bMock::~StarkC12bMock() free(pConstTreeAddress)" << endl;
    }
...
void StarkC12bMock::genProof(void *pAddress, FRIProofC12 &proof, Goldilocks::Element publicInputs[8])
{
    CommitPolsBasicC12b cmPols(pAddress, starkInfo.mapDeg.section[eSection::cm1_n]);
