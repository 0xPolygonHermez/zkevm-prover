...
#pragma omp parallel for
    ...
    {
        Polinomial fPol = starkInfo.getPolinomial(mem, starkInfo.exps_n[starkInfo.puCtx[i].tExpId]);
...
#pragma omp parallel for
    ...
    std::memcpy(&proof.publics[0], &publicInputs[0], starkInfo.nPublics * sizeof(Goldilocks::Element));

    nlohmann::ordered_json jProof = proof.proofs.proof2json();
    ...
    for (uint i = 0; i < proof.publics.size(); i++)
    {
        j_publics.push_back(Goldilocks::toString(proof.publics[i]));
    }
