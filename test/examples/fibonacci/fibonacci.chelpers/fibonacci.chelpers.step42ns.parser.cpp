...
#include "fibonacci.chelpers.step42ns.parser.hpp"
#include <immintrin.h>

#define AVX_SIZE_ 4

void FibonacciSteps::step42ns_parser_first_avx(StepsParams &params, uint64_t nrows, uint64_t nrowsBatch)
{
...
void FibonacciSteps::step42ns_parser_first(StepsParams &params, uint64_t nrows, uint64_t nrowsBatch)
{
#pragma omp parallel for
     for (uint64_t i = 0; i < nrows; i += nrowsBatch)
     {
...
void FibonacciSteps::step42ns_parser_first_avx_jump(StepsParams &params, uint64_t nrows, uint64_t nrowsBatch)
{
...
#ifdef __AVX512__
void FibonacciSteps::step42ns_parser_first_avx512(StepsParams &params, uint64_t nrows, uint64_t nrowsBatch)
{
#pragma omp parallel for
     for (uint64_t i = 0; i < nrows; i += nrowsBatch)
     {
