...
uint64_t HashDB64WorkflowTest (const Config& config)
{
    ...
    HashDB * localHashDB = hashDBSingleton.get();
    Database64 &db64 = localHashDB->db64;
    ...
    // Purge
    zkr = pHashDB->purge(batchUUID, fea2string(fr, root), persistence);
    zkassertpermanent(zkr==ZKR_SUCCESS);
    ...
    zkr = pHashDB->consolidateState(fea2string(fr, root), persistence, consolidatedStateRoot, flushId, storedFlushId);
    zkassertpermanent(zkr==ZKR_SUCCESS);
    zklog.info("CONSOLIDATE zkr=" + zkresult2string(zkr) + " virtualRoot=" + fea2string(fr, root) + " consolidatedRoot=" + consolidatedStateRoot + " flushId=" + to_string(flushId) + " storedFlushId=" + to_string(storedFlushId));

    // New state root
    string batchNewStateRootString = consolidatedStateRoot;
    Goldilocks::Element batchNewStateRoot[4];
    string2fea(fr, batchNewStateRootString, batchNewStateRoot);
    ...
    zkr = tree64.ReadTree(db64, batchNewStateRoot, keyValues, &hashValues);
    zkassertpermanent(zkr==ZKR_SUCCESS);
    zklog.info("READ TREE keyValues.size=" + to_string(keyValues.size()) + " hashValues.size=" + to_string(hashValues.size()));
